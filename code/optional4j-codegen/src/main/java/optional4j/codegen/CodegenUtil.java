...
import static optional4j.support.NullabilityValue.NON_NULL;
...
public class CodegenUtil {
    ...
    public static <T> void addNullableAnnotation(CtMethod<T> ctMethod, Factory factory) {
        if (ctMethod.hasAnnotation(Nullable.class)) {
            return;
        }
        ctMethod.addAnnotation(
                factory.createAnnotation()
                        .setAnnotationType(factory.createCtTypeReference(Nullable.class)));
    }

    public static NullabilityValue getNullness(
    ...
    public static ModeValue getMode(CtMethod<?> ctMethod, CodegenProperties codegenProperties) {
        ...
        Config config = ctMethod.getAnnotation((Config.class));
        if (config != null) {
            return config.mode();
        }

        config = ctMethod.getParent().getAnnotation(Config.class);
        if (config != null) {
            return config.mode();
        }
