...
public class CollaboratorVisitor extends CtAbstractVisitor {
    ...
    public <T> void visitCtClass(CtClass<T> ctClass) {
        ...
        if (ctClass.isSubtypeOf(optionalTypeBuilder.createOptionalOf(ctClass.getReference()))) {
            return;
        }
        ...
        // other classes
        ctClass.setSimpleName(ctClass.getSimpleName());
        ...
        // Modify ctClass
        optionalTypeBuilder.implementPresent(ctClass);
    ...
    private void declareInterfaceMethod(CtType<?> ctType, CtMethod<?> method) {
        ...
        CtTypeReference returnTypeRef = getReturnTypeRef(method);
        newMethod.setType(returnTypeRef);
                                                        ...
                                                        nullObjectBuilder
                                                                .createCodeSnippetExpression(
                                                                        Optional.class
                                                                                        .getSimpleName()
                                                                                + ".empty()")));
                            } else if (type.getSimpleName().startsWith("Abstract")
    ...
    private void implementNothingInterface(CtClass<?> ctClass, CtClass<?> aClass) {
        optionalTypeBuilder.implementNothing(ctClass, aClass);
    }
